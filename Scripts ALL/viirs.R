...
out_dir <- paste0(here(),"/Figures/")

# Load fire perimeter
fire_perim <- st_read(paste0(here(),"/Data/burnt_area_effis/Dava_fire_perimeter.shp"))
...
viirs_folder <- paste0(here(),"/Data/VIIRS/")
...
write.csv(all_points_df, paste0(here(),"/Data/viirs_frp_points.csv"), row.names = FALSE)
...
# Reproject to metres (UTM 30N) and make a perimeter buffer for cropping
pts_m         <- st_transform(all_points, 32630)
fire_perim_m  <- st_transform(fire_perim, 32630)
fire_buffer_m <- st_buffer(fire_perim_m, dist = 1000)   # 1 km buffer

# Daily concave hulls from POINTS (avoid MULTIPOINT to keep concaveman happy)
...
# CUMULATIVE footprint: union day-by-day, carefully building sfg list
...
# DAILY INCREMENTS: today minus yesterday, again building sfgs safely
