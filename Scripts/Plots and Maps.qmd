...
p_emis <- ggplot() +
  ...
  theme(
    ...
    legend.position      = c(0.54, 0.15),       # your current left alignment
...
#old colours-  "Low"       = "#FEE0D2",  # light red
#  "Moderate"  = "#FC9272",  # mid red
#  "High"    = "#CB181D"   # deeper, less neon than #FF0000

#new colours- c("Low"="#FEE08B","Moderate"="#F56D43","High"="#A50026")
...
# --- 1) Wide table by Category with totals and within-category % ---
cat_wide <- plot_df %>%
...
# --- 2) Totals by severity across all categories (with shares) ---
sev_totals <- plot_df %>%
  group_by(Severity) %>%
  summarise(Area_ha = sum(Area_ha, na.rm = TRUE), .groups = "drop") %>%
...
viirs_folder <- paste0(here(),"/Data/VIIRS_images")
...
# --- Ensure we have a WGS84 version of the layers used to plot -------------
# daily_inc: sf POLYGONS with a date (or date_f) column
# fire_perim_m: perimeter in metres (e.g., UTM); reproject to 4326 for axes
daily_inc_ll <- sf::st_transform(daily_inc, 4326)
perim_ll     <- sf::st_transform(fire_perim_m, 4326)

# Common bbox for tight framing
bb <- sf::st_bbox(perim_ll)

# --- Date factor + red palette ---------------------------------------------
if (!"date_f" %in% names(daily_inc_ll)) {
  dts <- sort(unique(as.Date(daily_inc_ll$date)))
  daily_inc_ll$date_f <- factor(daily_inc_ll$date,
                                levels = dts,
                                labels = format(dts, "%d %b"))
}
lvls  <- levels(daily_inc_ll$date_f)
reds3 <- c("#0072B2", "#E69F00", "#009E73")
pal   <- if (length(lvls) <= 3) reds3[seq_along(lvls)] else
         grDevices::colorRampPalette(reds3)(length(lvls))
mycol <- setNames(pal, lvls)

# --- Axes label helpers (deg + E/W, N/S) -----------------------------------
label_lon <- function(x) sprintf("%.3f°%s", abs(x), ifelse(x < 0, "W", "E"))
label_lat <- function(y) sprintf("%.3f°%s", abs(y), ifelse(y < 0, "S", "N"))

# --- Plot (layout cloned from p_sev) ---------------------------------------
p_inc2 <- ggplot() +
  geom_sf(data = daily_inc_ll, aes(fill = date_f),
          colour = "white", linewidth = 0.2, alpha = 0.9) +
  geom_sf(data = perim_ll, fill = NA, colour = "black", linewidth = 0.03) +
  scale_fill_manual(values = mycol, name = "Fire spread (VIIRS)\nby date", drop = FALSE) +
  coord_sf(
    crs  = sf::st_crs(4326),
    xlim = c(bb["xmin"], bb["xmax"]),
    ylim = c(bb["ymin"], bb["ymax"]),
  ...
  theme(
    ...
    panel.background     = element_rect(fill = "white", colour = NA),
    plot.background      = element_rect(fill = "white", colour = NA),
    # Legend inside, bottom-right, vertical (same sizing/box as p_sev)
  ...
  # Scale bar: bottom-centre (anchor at BL + big pad_x), same as p_sev
  ggspatial::annotation_scale(
    location   = "bl",
    width_hint = 0.28,
    pad_x      = unit(0.58, "npc"),
  ...
  # North arrow: bottom-right, simple orienteering style, same size/offsets
  ggspatial::annotation_north_arrow(
    location = "br", style = north_arrow_orienteering,
    pad_x = unit(0.3, "cm"), pad_y = unit(0.2, "cm"),
    height = unit(0.5, "cm"), width = unit(0.3, "cm")
  )

print(p_inc2)

# --- (Optional) Save to EMF (A4 landscape width) ---------------------------
library(devEMF)
out_emf <- "C:/Users/jscho/OneDrive - University of Cambridge/Moorayshire Wildfire Data/Figures/daily_increments_layout_like_sev.emf"
ggsave(
  filename  = out_emf,
  plot      = p_inc2,
  device    = function(filename, ...) devEMF::emf(filename, emfPlus = TRUE, family = "Arial", ...),
  width     = 210, height = 150, units = "mm",   # A4 landscape
  bg        = "white",
  limitsize = FALSE
)
