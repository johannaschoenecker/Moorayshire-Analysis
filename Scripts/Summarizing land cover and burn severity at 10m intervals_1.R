...
# ---- Read raster (use band 1 if multi-band) ----
r <- terra::rast(tif_path)
if (terra::nlyr(r) > 1) r <- r[[1]]
r_crs <- terra::crs(r)
cat("Raster CRS:", r_crs, "\n")

# ---- Read centres (CSV must have columns x and y; ID optional) ----
...
# Total sampled (non-NA) points per centre
...
# Compute fractions only overa the class_* columns
...
# ---- 2) Reproject the sampling points to the dNBR CRS (fast) and extract ----
...
# ---- 5) Wide tables (handy for joins and spreadsheets) ----
...
####### Calculate belowground emissions for Peatlands and Moorlands

# Variables
bulk_density_peat <- 58.2 #58.2 #19.29 #68.64 # From Abernethy
bulk_density_moorlands <- 58.2 #187.65 # From Glenmore
